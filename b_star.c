...
void b_star(int key, const char* file_name, int qtd, bool p) {
   ...
   if (p){
   	...
   	searchBStar(&reg, &tree, &transfer, &compare);
    end = clock();
	printf("================== Analise: Busca ==================\n");
    printf("COMPARACOES             : %ld \n", compare);
    printf("TRANSFERENCIAS          : %ld \n", transfer);
...
void searchBStar(Register* it, PointerS *p, long int* transfer, long int* compare) {
    int i;
    ...
    if ((*p)->st == INTERNAL){
		...
		*compare += 2;
        ...
        if (it->key < pag->UU.UI.key_int[i - 1]){
			searchBStar(it, &pag->UU.UI.p_int[i - 1], transfer, compare);
		}

        else{
			searchBStar(it, &pag->UU.UI.p_int[i], transfer, compare);
		}
        return;
    }
